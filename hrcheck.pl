  ...
  for ($a) {
    /^[0-9]+:[0-9]+/ && do {
      ...
      if ( $time[0] > 24 or $time[0] < 0 ) {
        warn("Odd hour value, going to do the best I can.\n");
      }
      $hourTemp = $time[0] % 24;
      $time[1] =~ s/[^0-9]*$//;
      if ( $time[1] > 60 or $time[1] < 0 ) {
        warn("Odd minute value, going to do the best I can.\n");
      }
    ...
    /^(-|\+)?[0-9]+$/ && do {
      $adjust = $a;
      print
"Adjusting time from current by $adjust minutes. Use : to try an exact time.\n";
      $count++;
      next;
    };
    ...
    /^=/i
      && do { $bookmarkLook = $a; $bookmarkLook =~ s/^=//; $count++; next; };
    /^-?bp$/i && do {
      for ( $check, $check2, $xtraFile ) { printBkmk($_); }
      exit();
    };
    usage();
  }
...
if ( ($popupIfAbort) && ( !$gotImportantLine ) ) {
  Win32::MsgBox(
"Remember to remove any semicolons for debugging, so HRCHECK reads in everything!"
  );
}
...
if ( !$anyExtra ) {
  print("For critical stuff to run, you may wish to run -x.\n");
}
else {
  print(
  ...
  while ( $line = <A> ) {
    ...
    for $thistime (@times) {
      ...
      for my $q ( keys %browsMap ) {
        ...
        if ( $name_idx >= 0 ) {
          substr( $b[$cmdCount], $name_idx, length($q), $browsMap{$q} );
        }
      }
      if ( validHour($thistime) ) {
        ...
        if ( ( $b[0] eq "*" )
          || ( $qhr[$min] || $tens[$min] || ( $thistime < 0 ) )
          || $autoBookmark )
        {
          if ( -f "$b[$cmdCount]"
            && ( $b[$cmdCount] =~ /(txt|otl)$/i ) )  # skip over empty text file
          {
            if ( -s "$b[$cmdCount]" == 0 ) {
              print "Skipping empty file $b[$cmdCount].\n";
              next;
            }
          }
          ...
          if ($printOnly) {
            ...
            if ( $ignoreHiddenBookmark && !$allBookmarks ) {
              print "Not running $b[$cmdCount]"
                . ( $bookmarkNote ? "" : " (-ab to run this and others)" )
                . "\n";
              $bookmarkNote = 1;
            }
            else {
              print "Running "
                . ( $system ? "(nonsequentially) " : "" )
                . "$b[$cmdCount]\n";
              if ($system) {
                system 1, $b[$cmdCount];
              }
...
sub printBkmk {
  open( A, $_[0] )
    || do { warn("$_[0] not found as an hrcheck file.\n"); return; };
  while ( $a = <A> ) {
    if ( $a =~ /^=[^=]/ ) {
      chomp($a);
      if ( $a =~ /^=\// ) { print "HIDDEN: "; }
