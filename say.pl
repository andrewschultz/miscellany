...
while ($#ARGV > -1)
{
  if ($ARGV[0] =~ /,/) { my @ary = split(/,/, $ARGV[0]); for my $dir (@ary) { searchSay($dir); } }
  elsif ($ARGV[0] eq "-a")
  {
...
sub searchSay
{
  ...
  if (($_[0] eq "roiling") || ($_[0] eq "sa"))
  {
    ...
    searchOne("c:/Program Files (x86)/Inform 7/Inform7/Extensions/Andrew Schultz/$_[0] Random Text.i7x", 2);
	searchOne("c:/Program Files (x86)/Inform 7/Inform7/Extensions/Andrew Schultz/$_[0] Nudges.i7x", 3);
  }
