...
if len(file_args) != 2:
    print("You need 2 file arguments: 1st the project, 2nd the module description. You have {:d}.".format(file_args))
    usage()

base_file_noxt = '{:s} {:s}'.format(i7.proj_exp(file_args[0], False), i7.hf_exp(file_args[1])).title().replace('-', ' ')
base_file = base_file_noxt + ".i7x"

if base_file.startswith(" "): sys.exit("You need to define a valid project or project abbreviation.")

if os.path.exists(i7.extdir): print("Yay! Extension directory {:s} found!".format(i7.extdir))
...
if os.path.exists(x) and not overwrite:
    ...
    f = open(x, "w")
    f.write("Version 1/{:02d}{:02d}{:02d} of {:s} by {:s} begins here.\n\n\"This is a basic comment.\"\n\n".format(now.year % 100, now.month, now.day, base_file_noxt, i7.auth))
