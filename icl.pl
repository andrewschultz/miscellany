...
while ($x = <X>)
{
  ...
  elsif ($x =~ /^z6:/) { $y = $x; $y =~ s/^z6://g; $zmac{p($y)} = 6; } #z6 is hacky code for z5 in release/beta but z8 in debug
  elsif ($x =~ /^z7:/) { $y = $x; $y =~ s/^z7://g; $zmac{p($y)} = 7; } #z7 is hacky code for z5 in release but z8 in debug/beta
