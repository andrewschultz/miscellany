...
while ( my $line = <A> ) {
  if ( $line =~ /;format:gf-markup/ ) {
    die("This has already been converted. Delete the first line to try again.");
  }
  if ( $line =~ /  OUTLINE/ ) {
    die(
"You may want to delete your manual outline so that the ='s are not duplicated."
    );
  }
  $line =~
s/schultza\@earthlink.net/blurglecruncheno\@gmali.cmo \(anti spam letter flip\)/g;
  ...
  $line =~
s/schultw.andrez\@sbcglobal.net/blurglecruncheno\@gmail\.cmo \(anti spam letter flip\)/g
    ;    # change email address
  $line =~
s/schultz.andrew\@sbcglobal.net/blurglecruncheno\@gmail\.cmo \(anti spam letter flip\)/g
    ;    # change email address
  $line =~
s/schultza\@earthlink.net/blurglecruncheno\@gmail\.cmo \(anti spam letter flip\)/g
    ;    # change email address
  if ( ( $line =~ /[a-z\. '\\"\*\_]/i )
    && ( length($line) > 65 )
    && ( $line !~ /^[\#\*\|]/ ) )
...
printf(
  "%d bytes to %d bytes, total spaces = %d.\n",
  -s "$filename",
  -s "$outfile",
  $spacesFound
);
